...
<template>
  <teleport to="body">
    <div class="fixed inset-0 z-[9999] flex items-center justify-center px-4">
      ...
      <div class="relative w-full max-w-4xl bg-white rounded-2xl p-6">
        <h2 class="text-xl font-semibold mb-4">
          {{ mode === 'add' ? 'Add Test' : 'Edit Test' }}
        </h2>
...
const emit = defineEmits<{
  (e: 'save', updatedTest: any, type: 'listening' | 'reading' | 'writing'): void
  (e: 'close'): void
}>()

// Make a reactive copy of the test
const editingTest = reactive({ ...props.test })

// Watch prop changes
watch(
    () => props.test,
    (val) => {
      if (val) Object.assign(editingTest, val)
    },
...
// Save handler
const saveChanges = () => {
  emit('save', editingTest, props.type)
}
</script>
